...
func _ready() -> void:
	hide()
	# Capture each NPC's system prompt and clear it from the node.
	# We prepend it manually to the first user message so it works with any
	# model regardless of whether the model supports a system chat role.
...
func open_chat() -> void:
	...
	if met_flag != "" and met_flag in GameState.ALLOWED_FLAGS and not GameState.has_flag(met_flag):
...
func send_text_to_ai() -> void:
	var message := text_edit.text.strip_edges().left(MAX_PLAYER_INPUT)
	if message == "":
		return
